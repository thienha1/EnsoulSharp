...
using SharpDX;
using SPrediction;
using MinionManager = EnsoulSharp.Common.MinionManager;
...
namespace Nechrito_Gragas
{
    class Program
    {
        ...
        {
            if (Player.CharacterName != "Gragas") return;
        ...
        private static void OnDoCast(AIBaseClient sender, AIBaseClientProcessSpellCastEventArgs args)
        {
            ...
            if (args.Target is AIMinionClient)
            {
                ...
                {
                    var minions = MinionManager.GetMinions(Player.PreviousPosition, 600);
                    {
                        if (minions == null)
                            return;
        ...
        public static AIBaseClient GetCenterMinion()
        {
        ...
        protected static void SmiteJungle()
        {
            foreach (var minion in MinionManager.GetMinions(900f, MinionTypes.All, MinionTeam.Neutral))
            {
                var damage = Player.Spellbook.GetSpell(Spells.Smite).State == SpellState.Ready
                    ? (float)Player.GetSummonerSpellDamage(minion, Damage.DamageSummonerSpell.Smite)
                    : 0;
                if (minion.Distance(Player.PreviousPosition) <= 550)
                {
                    if ((minion.CharacterData.Name.Contains("Dragon") || minion.CharacterData.Name.Contains("Baron")))
                    {
