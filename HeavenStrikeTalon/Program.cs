...
namespace HeavenStrikeTalon
{
    ...
    public static class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            EnsoulSharp.Drawing.OnDraw += Drawing_OnDraw;
        ...
        private static void Game_OnGameUpdate(EventArgs args)
        {
            ...
            {
                if (Selected() && OutOfAA(TargetSelector.SelectedTarget) && Orbwalking.CanMove(80))
                {
        ...
        {
            if (args.Target is BuildingClient || args.Target is HQClient
                || args.Target is Barracks || args.Target is BarracksDampenerClient)
                return;
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
        ...
        {
            if (target is BuildingClient || target is HQClient
                || target is Barracks || target is BarracksDampenerClient)
                return;
            if (Orbwalker.ActiveMode == Orbwalking.OrbwalkingMode.Combo)
            {
