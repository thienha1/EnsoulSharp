    ...
    internal class Program
    {
        ...
        private static void SemiAutomaticE()
        {
            if (E.IsReady() && Config.Item("key.e").GetValue<KeyBind>().Active)
            {
                if(Config.Item("key.emode").GetValue<StringList>().SelectedIndex == 1)
                {
                    ...
                    {
                        if (barrel.GetBarrel().Distance(Game.CursorPosRaw) < lowest)
                        {
                            bPos = barrel.GetBarrel().Position;
                            lowest = barrel.GetBarrel().Distance(Game.CursorPosRaw);
                        }
                    }
                    if (lowest != 1600f)
                    {
                        E.Cast(bPos.Extend(Game.CursorPosRaw, Math.Min(685, lowest)));
                    }
                }
                else if(Config.Item("key.emode").GetValue<StringList>().SelectedIndex == 2 && Q.IsReady())
                {
                    ...
                    if (toExplode.Any())
                    {
                        ...
                        if(bar != null)
                        {
                            E.Cast(bar.GetBarrel().Position.Extend(Game.CursorPosRaw, Math.Min(685, lowest)));
        ...
        private static void DrawE()
        {
            if (E.IsReady() && Config.Item("drawings.ex").GetValue<bool>())
            {
                ...
                if (lowest != 1600f)
                {
                    Render.Circle.DrawCircle(bPos.ExtendToMaxRange(Game.CursorPosRaw,685),350,Color.ForestGreen);
                    Drawing.DrawLine(Drawing.WorldToScreen(bPos),Drawing.WorldToScreen(bPos.ExtendToMaxRange(Game.CursorPosRaw,685)),5,Color.ForestGreen);
                }
            }
        }
