...
namespace JustShyvanaV2
{
    class Program
    {
        ...
        static void UseQ(AIBaseClient target)
        {
            if (Q.IsReady() && target.Distance(Player.PreviousPosition) <= Q.Range)
            {
                if (Q.Cast())
        ...
        {
            if (W.IsReady() && target.Distance(Player.PreviousPosition) <= W.Range && !Player.HasBuff("ShyvanaTransform"))
            {
                W.Cast();
            }

            if (W.IsReady() && target.Distance(Player.PreviousPosition) <= 350 && Player.HasBuff("ShyvanaTransform"))
            {
                W.Cast();
            }
        }

        static void UseE(AIBaseClient target)
        {
            if (E.IsReady() && target.Distance(Player.PreviousPosition) <= E.Range)
            {
        ...
        static void UseR(AIHeroClient target)
        {
            ...
            if (minr <= enemys && R.IsReady() && target.Distance(Player.PreviousPosition) <= R.Range)
            {
                R.CastIfHitchanceEquals(target, HitChance.Medium);
            }
        }

        private static bool IsLethal(AIBaseClient unit)
        {
            return ComboDamage(unit) / 1.65 >= unit.Health;
        }

        private static double ComboDamage(AIBaseClient unit)
        {
            if (unit == null)
                return 0d;
