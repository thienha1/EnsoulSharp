...
namespace HyperCarryNautilus
{
    internal class Nautilus : Spells
    {
        ...
        private void OnLoad(EventArgs args)
        {
            ...
            Interrupters.OnInterrupter += OnInterruptable;
        ...
        private void ExecuteCombo()
        {
            ...
            if (castR && ConfigMenu.config.Item(target.CharacterData.SkinName) != null && ConfigMenu.config.Item(target.CharacterData.SkinName).GetValue<bool>() == false)
            {
                r.CastOnUnit(target);
            }
        }
        ...
        private void KillSteal()
        {
            ...
            {
                foreach (var target in HeroManager.Enemies.Where(enemy => enemy.IsValidTarget(ignite.SData.CastRange) && !enemy.HasBuffOfType(BuffType.Invulnerability)).Where(target => target.Health < ObjectManager.Player.GetSummonerSpellDamage(target, Damage.DamageSummonerSpell.Ignite)))
                {
