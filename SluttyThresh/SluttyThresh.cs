...
using LCItems = EnsoulSharp.Common.Items;
...
namespace Slutty_Thresh
{
    internal class SluttyThresh : MenuConfig
    {
        ...
        public static void OnLoad(EventArgs args)
        {
            ...
            Interrupters.OnInterrupter += ThreshInterruptableSpell;
        ...
        private static void Itemusage()
        {
            ...
            {
                if (Config.Item("faceop" + hero.CharacterName).GetValue<StringList>().SelectedIndex == 0)
                {
                    if (hero.HealthPercent <= Config.Item("facehp" + hero.CharacterName).GetValue<Slider>().Value)
                    {
            ...
            foreach (var hero in
                HeroManager.Allies.Where(x => !x.IsMe))
            {
                if (Config.Item("locketop" + hero.CharacterName).GetValue<StringList>().SelectedIndex == 0)
                {
                    if (hero.HealthPercent <= Config.Item("lockethp" + hero.CharacterName).GetValue<Slider>().Value)
                    {
            ...
            {
                if (Config.Item("healmikaels" + hero.CharacterName).GetValue<StringList>().SelectedIndex == 0)
                {
           ...
           // AIHeroClient target = TargetSelector.GetTarget(Q.Range, TargetSelector.DamageType.Magical);
            ...
            foreach (var hero in
                HeroManager.Allies.Where(x => !x.IsMe
                                              && !x.IsDead))
            {
                if (Config.Item("healop" + hero.CharacterName).GetValue<StringList>().SelectedIndex == 0)
                {
                    if (hero.HealthPercent <= Config.Item("hpsettings" + hero.CharacterName).GetValue<Slider>().Value
            ...
            switch (Config.Item("combooptions").GetValue<StringList>().SelectedIndex)
            {
                ...
                case 1:
                    if (target.IsValidTarget(E.Range)
                        && Environment.TickCount - lastq >= 40 + Game.Ping 
                        && eSpell)
                        E.Cast(target.Position.Extend(Player.PreviousPosition,
        ...
        private static void Unit_OnDash(AIBaseClient sender, Dash.DashItem args)
        {
            ...
            {
                if (E.IsReady()
                   && E.IsInRange(sender.PreviousPosition))
                {
        ...
        private static void Drawing_OnDraw(EventArgs args)
        {
                ...
                && target.Distance(Player) >= Q.Range - 200)
            {
                ...
                var textDimension = Drawing.GetTextEntent("Stunnable!", 25);
