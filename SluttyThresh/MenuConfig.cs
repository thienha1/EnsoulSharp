...
namespace Slutty_Thresh
{
    ...
    new MenuItem("rslider", "R Only if X Enemies in Range").SetValue(new Slider(3, 1, 5)));
            ...
            var lantMenu = new Menu("Lantern Settings", "lantern");
            {
                ...
                {
                    {
                        lantMenu.AddItem(new MenuItem("healop" + hero.CharacterName, hero.CharacterName))
                            .SetValue(new StringList(new[] {"Lantern", "No Lantern"}));

                        lantMenu.AddItem(
                            new MenuItem("hpsettings" + hero.CharacterName, "Lantern When % HP <").SetValue(
                                new Slider(20)));
                    }
            ...
            var shieldMenu = new Menu("Shield Usage", "shield");
            {
                var mountainmenu = new Menu("Face Of The Mountain", "faceof");
                {
                    ...
                    {
                        {
                            mountainmenu.AddItem(new MenuItem("faceop" + hero.CharacterName, hero.CharacterName))
                                .SetValue(new StringList(new[] {"Use", "Don't Use"}));

                            mountainmenu.AddItem(
                                new MenuItem("facehp" + hero.CharacterName, "Use When % HP <").SetValue(new Slider(20)));
                        }
                    }
                }
                ...
                var locketmenu = new Menu("Locket Of Solari", "locksol");
                {
                    ...
                    {
                        {
                            locketmenu.AddItem(new MenuItem("locketop" + hero.CharacterName, hero.CharacterName))
                                .SetValue(new StringList(new[] {"Use", "Don't Use"}));

                            locketmenu.AddItem(
                                new MenuItem("lockethp" + hero.CharacterName, "Use When % HP <").SetValue(
                                    new Slider(20)));
                        }
                    }
            ...
            var healMenu = new Menu("Healing Items", "heals");
            {
                ...
                {
                    {
                        allies.AddItem(new MenuItem("healmikaels" + hero.CharacterName, hero.CharacterName))
